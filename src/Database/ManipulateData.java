...
public abstract class ManipulateData {
	...
	public HashMap<String, ArrayList<String>> getAllData() {
	    HashMap<String, ArrayList<String>> allData = new HashMap<>();
	    
	    try {
	        // Execute a select all query on the current table
	        connect.rs = connect.execQuery("SELECT * FROM " + table);
	        ...
	        while (connect.rs.next()) {
	            ...
	            for (int i = 1; i <= columnCount; i++) {
	                ...
	                allData.put(columnName, this.getData(columnName));
	        }
	        }
	    } catch (SQLException e) {
	        System.err.println("Error retrieving data from table: " + table);
	        e.printStackTrace();
	    }
